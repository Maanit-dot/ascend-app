...
function StatPill({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="flex items-center justify-between gap-1 rounded-md border border-arc-500/15 bg-void/60 px-2 py-0.5">
      <span className={cn("font-mono text-[8px] uppercase tracking-wider", color)}>{label}</span>
      <span className="font-mono text-[10px] font-bold text-ink-secondary">{value}</span>
    </div>
  );
}
...
export function Sidebar() {
  ...
  const strVal = character?.stats?.strength ?? 10;
  const agiVal = character?.stats?.agility ?? 10;
  const intVal = character?.stats?.knowledge ?? 10;
  ...
  return (
    <aside className="z-40 hidden h-full w-64 flex-shrink-0 flex-col border-r border-arc-500/20 bg-void md:flex overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto min-h-0 px-3 py-3 scrollbar-thin scrollbar-thumb-arc-900/40">
        <p className="mb-2 px-2 system-label">MAIN PORTAL</p>
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                ...
                />
                <span className={cn("flex-1 text-xs", isActive && "font-semibold")}>{item.label}</span>

                {/* Badge if available */}
                {item.badge && (
                  <span className="rounded bg-arc-500/20 border border-arc-500/40 px-1 py-0.2 font-mono text-[8px] text-arc-300 font-bold">
                    {item.badge}
                  </span>
                )}
      ...
      {user && character && (
        <div className="flex-shrink-0 border-t border-arc-500/20 bg-arc-950/40 p-3 space-y-2">
          {/* Avatar + User details */}
          <div className="flex items-center gap-2.5">
            <div className="relative flex-shrink-0">
              ...
              <div className="relative flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br from-arc-600 to-arc-900 border border-arc-500/40 overflow-hidden shadow-glow-arc-sm">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5">
                ...
                <span className="flex items-center gap-0.5 font-mono text-[8px] text-emerald-400 font-semibold">
                  <span className="h-1 w-1 rounded-full bg-emerald-400" /> ONLINE
                </span>
              </div>
              <p className="font-mono text-[9px] font-medium text-arc-300 truncate mt-0.5">
                {currentTitle}
              </p>
              <span className="inline-block mt-0.5 font-mono text-[8px] text-ink-faint">
          ...
          {/* Level badge + XP */}
          <div className="rounded-lg border border-arc-500/20 bg-void/50 p-1.5">
            <div className="flex items-center justify-between mb-1">
              <span className="system-label">LEVEL</span>
              <span className="font-display text-base font-bold text-arc-400 leading-none text-glow-arc">
                {character.level}
              </span>
            </div>
            {/* XP bar */}
            <div className="mb-0.5 flex justify-between font-mono text-[8px] text-ink-faint">
          ...
          <div className="flex items-center justify-around pt-1 border-t border-arc-500/15">
            {[Share2, Globe, Bell, Settings].map((Icon, i) => (
              <button
                key={i}
                className="flex h-6 w-6 items-center justify-center rounded-lg text-ink-faint hover:text-arc-400 hover:bg-arc-500/10 transition-colors"
              >
